...
async function msgHandler (client, message) {
    try {
        ...
        case 'menu':
        case 'help': {
            const text = `👋️ Hi, ${pushname}! \n\nGunakan = #\nUntuk memulai perintah!\n\n*STICKER*\n\nKetik #sticker\nUntuk merubah gambar menjadi stiker,kirim gambar dengan caption #sticker atau balas gambar yang sudah dikirim dengan #sticker\n\n*Downloader* (Video only)\n\nKetik: #tiktok(sepasi)urlvideo\nUntuk mendownload video dari Tiktok\n\nKetik: #ig(sepasi)urlvideo\nUntuk mendownload video dari ig\n\nKetik: #twt(sepasi)urlvideo\nUntuk  mendownload video dari Twitter\n\nHope you have a great day!✨`
        ...
        case 'tiktok': {
            ...
            await client.sendText(from, '*Sedang Memproses....*')
            await tiktok(url)
                .then((videoMeta) => {
                    const filename = videoMeta.authorMeta.name + '.mp4'
                    const caps = `*Metadata:*\nUsername: ${videoMeta.authorMeta.name} \nMusic: ${videoMeta.musicMeta.musicName} \nView: ${videoMeta.playCount.toLocaleString()} \nLike: ${videoMeta.diggCount.toLocaleString()} \nComment: ${videoMeta.commentCount.toLocaleString()} \nShare: ${videoMeta.shareCount.toLocaleString()} \nCaption: ${videoMeta.text.trim() ? videoMeta.text : '-'} \n\nBantu kami merawat server dengan donasi di https://saweria.co/donate/JagaServerBot
Terimakasih`
