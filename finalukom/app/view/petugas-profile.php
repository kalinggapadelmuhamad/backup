...
<?php if (isset($_GET['menu'])) {
	...
	if ($menu == 'edit') { ?>
		...
		<form method="POST" action="../app/data/petugas-sys-update-petugas.php">
			<center>
			...
			<table class="table center mt-2">
				...
				<tr>
					<td>Level</td>
					<td><input readonly="" class="form-control" type="text" name="level" value="<?= $_SESSION['level'] ?>"></td>
				</tr>
			</table>
				<button type="submit" name="submit" class="btn btn-success btn-md"><i class="fas fa-paper-plane"></i></button>
				<a href="?halaman=petugas-profile" class="btn btn-warning btn-md text-white"><i class="fas fa-times"></i></a>
			</div>
		</form>
...
<?php	}
	...
	<table class="table center mt-4">
		...
		<tr>
			<td>Level</td>
			<td><input class="form-control" type="text" name="" readonly="" value="<?= $_SESSION['level'] ?>"></td>
		</tr>
	</table>
	<a href="?halaman=petugas-profile&menu=edit" class="btn btn-primary btn-md"><i class="fas fa-user-edit"></i></a>
